...
function toFrontendClient(client: any) {
  ...
  const overdueAmount = contracts
    ...
    .reduce((sum: number, contract: any) => sum + (contract.contractValue ?? 0), 0);
  ...
  return {
    ...
    totalOutstanding,
    overdueAmount,
    nextRenewal: nextContract ? formatDate(new Date(nextContract.renewalDate)) : "—",
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const contactPerson = body.contactPerson?.trim() || body.contact?.trim() || null;
    const email = body.email?.trim() || null;
    const phone = body.phone?.trim() || null;
    const address = body.address?.trim() || body.billingAddress?.trim() || null;
